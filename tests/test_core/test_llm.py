...
async def test_chat_retries_on_malformed_200_and_raises_after_exhaustion(
    ...
    Vultr Inference occasionally returns HTTP 200 with a semantic error
    ...
    transport = httpx.MockTransport(handler)
    # Vultr cascade: primary (MiniMaxAI/MiniMax-M2.7-normalize) + 1
    # fallback (Qwen/Qwen3.6-27B-FP8) = 2 models, each with max_retries=2
    # attempts, so total calls = 4.
    async with LlmClient(client=_build_client(transport), max_retries=2) as llm:
        with pytest.raises(LlmError, match="missing choices"):
            await llm.chat([{"role": "user", "content": "Hi"}])
    expected = 4  # 2 models x 2 retries
