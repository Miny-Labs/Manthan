...
A single async loop where the upstream LLM (Vultr Serverless Inference,
MiniMax M2.7 by default) reasons, emits tool calls, observes results,
and iterates. Rich SSE events emitted at every decision point so any
frontend can render a live, transparent agent experience.
...
class ManthanAgent:
    ...
    def __init__(self, config: AgentConfig | None = None) -> None:
        ...
        api_key = self.config.vultr_api_key
        if not api_key:
            from src.core.config import get_settings

            api_key = get_settings().vultr_api_key.get_secret_value()
        self.router = ToolRouter(self.config)
        self._llm = httpx.AsyncClient(
            base_url=self.config.vultr_base_url,
            headers={
    ...
    async def _call_llm(
        ...
        """Call Vultr Inference with the current messages + tools."""
